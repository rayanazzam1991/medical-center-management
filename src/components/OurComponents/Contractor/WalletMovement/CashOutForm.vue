  ...
  setup(props, context) {
    ...
    const onSubmitAdd = handleSubmit(async (values) => {
      ...
      if (success) {
        ...
        router.push({ path: `/contractor/${selectedContractor.value}` ,  
                       query: { tab: 'Wallet' }
});
      } else {
        await sleep(200);
    ...
    return { t,contractorName,contractorId,Amount, selectedContractor,contractorList,pageTitle, onSubmit, currentWalletMovement, viewWrapper, backRoute, WalletMovementConsts, walletMovementStore };
  },
                                ...
                                <VField id="wallet_id">
                                    ...
                                    <VControl>
                                        <VSelect v-model="selectedContractor">
                                            ...
                                            <VOption v-for="contarctor in contractorList" :key="contarctor.id"
                                                :value="contarctor.id">
                   ...
                   <!--Fieldset-->
                    <div class="form-fieldset">
                        <div class="columns is-multiline">
                            <div class="column is-12">
                                <VField  v-slot="{ field }">
                                    <VLabel > {{t('walletMovement.cash_out_form.my_total')}}</VLabel>
                        ...
                        <div class="columns is-multiline">
                            <div class="column is-12">
                                <VField id="total" v-slot="{ field }">
                                    <VLabel class="required">{{t('walletMovement.cash_out_form.cash_out_total')}}</VLabel>
                                    <VControl icon="feather:dollar-sign">
