...
import {Employee, EmployeeService} from '/@src/models/Employee/employee';
...
import {defaultWaitingList, TicketServicesNotesHelper, WaitingList} from '/@src/models/Sales/WaitingList/waitingList';
import {moveTicketToNextWaitingList} from '/@src/services/Sales/Ticket/ticketService';
import {
  createTicketService,
  deleteTicketService,
  serveTicketService
} from '/@src/services/Sales/TicketService/ticketServiceService';
import {
  getWaitingListByProvider,
  serveNextTicketInProviderWaitingList
} from '/@src/services/Sales/WaitingList/waitingListService';
...
<template>
  <div class="header is-flex is-justify-content-space-between is-align-items-center">
    ...
    <VField v-permission="Permissions.EMPLOYEE_AVAILABILITY_TOGGLE" class="is-align-self-start">
      <VControl>
        <VSwitchSegment v-model="isAvailable"
                        :disabled="waitingListStore.loading || isThereServingTicket || disableAvailabilitySwitch" :label-false="t('employee.waiting_list.is_not_available')"
                        :label-true="t('employee.waiting_list.is_available')"
  ...
  <div class="waiting-list-outer-layout is-flex is-align-items-center is-justify-content-center ">
    ...
    <div v-else-if="employeeWaitingList.waiting_list.length > 0" class="waiting-list-inner">
      <div class="waiting-lists-container is-flex has-slimscroll">
        <WaitingListComponent :key="keyIncrement" :current_turn_number="employeeWaitingList.current_turn_number"
                              :provider="employeeWaitingList.provider"
                              :waiting_list="employeeWaitingList.waiting_list"/>
      </div>
    </div>
    <div v-else>
      <div class="columns is-multiline placeholder">
        <div ref="markdownContainer" class="column doc-column is-12">
          <VPlaceholderPage :subtitle="t('employee.waiting_list.no_data.subtitle')" :title="t('employee.waiting_list.no_data.title')"
                            class="placeholder" larger>
            <template #image>
              <img alt="" class="light-image"
    ...
    <div class="ticket-details-layout has-slimscroll ">
      <div class="ticket-details-inner is-flex-direction-column is-justify-content-space-between  ">
        <div v-if="isThereServingTicket" class="ticket-content is-flex-grow-1">
              ...
              <div v-for="requested_service in servingTicket.requested_services" :key="requested_service.id"
                ...
                <div class="service-details column is-4  mb-0 pb-0">
                  ...
                  <div v-else>
                    <VField>
                      <VControl>
                        <VTextarea v-model="requested_service.note"
                                   :placeholder="t('employee.waiting_list.type_notes')" rows="1"
                                   @input="() => updateTicketNotesHelper(requested_service.id)">
                        </VTextarea>
                      </VControl>
                    </VField>
                  </div>
                ...
                <div class="service-details column is-2  mb-0 pb-0">
                  <VButton v-if="requested_service.provider.id == employeeId && requested_service.status == TicketServiceConsts.NOT_SERVED"
                           v-permission="Permissions.SHOW_WAITING_LIST_SERVE_CLIENT" color="primary" raised
                           @click="serveConfirmation(requested_service.id)">
                ...
                <div class="service-details column is-2 pt-0">
                  <VIconButton v-if="requested_service.provider.id == employeeId && requested_service.status == TicketServiceConsts.NOT_SERVED"
                               v-permission="Permissions.TICKET_SERVICE_DELETE"
                               color="danger" icon="feather:trash-2" @click="removeService(requested_service.id)">
                  </VIconButton>
                </div>
              </div>
        ...
        <div v-else class="is-flex is-justify-content-center margin-center">
          ...
          <VButton v-if="!isThereServingTicket" v-permission="Permissions.SHOW_WAITING_LIST_SERVE_CLIENT"
                   :disabled="employeeWaitingList.waiting_list.length == 0 || !employeeWaitingList.provider.is_available" :loading="waitingListStore.loading" class="center" color="primary"
                   raised
                   @click="serveNext">
            {{
        ...
        <div v-if="isThereServingTicket" class="ticket-footer">
          <div class="ticket-footer-inner">
            <VButton v-if="isThereServingTicket" v-permission="Permissions.TICKET_SERVICE_CREATE"
                     :loading="waitingListStore.loading" color="primary" outlined
                     @click="addNewService"> {{
                t(`employee.waiting_list.add_new_service`)
              }}
            </VButton>
            <VButton v-if="isThereServingTicket"
                     v-permission="Permissions.SHOW_WAITING_LIST_SERVE_CLIENT" :loading="waitingListStore.loading" color="primary" raised
                     @click="ticketServingDone"> {{
                t(`employee.waiting_list.done`)
              }}
            </VButton>
          </div>
        </div>
  ...
  <transition name="slide">
    <div :class="[alertCount == 1 && 'is-warning', alertCount > 1 && 'is-danger', !alertMessage && 'alert-provider-notif-reverse', alertMessage && 'alert-provider-notif-slide']"
         class="alert-provider-notif">
      <VIconButton
  ...
  <VModal :open="serveServiceConfirmationPopup" :title="t('employee.waiting_list.serve_confirmation.title')"
          actions="center" @close="serveServiceConfirmationPopup = false">
    <template #content>
      <VPlaceholderSection :subtitle="t('employee.waiting_list.serve_confirmation.subtitle')"
                           :title="t('employee.waiting_list.serve_confirmation.caution')"/>
    </template>
  ...
  <VModal :open="deleteServiceConfirmationPopup" :title="t('employee.waiting_list.delete_confirmation.title')"
          actions="center" @close="deleteServiceConfirmationPopup = false">
    <template #content>
      <VPlaceholderSection :subtitle="t('employee.waiting_list.delete_confirmation.subtitle')"
                           :title="t('employee.waiting_list.delete_confirmation.caution')"/>
    </template>
    <template #action="{ close }">
  ...
  <VModal :key="keyIncrementAddServiceModal" :open="addServicePopup"
          :title="t('employee.waiting_list.add_service_modal.title')" actions="center" @close="addServicePopup = false">
    <template #content>
      <div class="form-fieldset">
        <div class="columns is-multiline">
          <div class="column is-12 columns">
            <VField id="user_status_id" class="column is-9">
            ...
            <VField id="user_status_id" class="column is-3">
              <VLabel>{{ t('employee.waiting_list.add_service_modal.service_price') }}</VLabel>
              <VControl>
                <VInput v-model="selectedServiceProviderPrice" disabled type="number"/>
              </VControl>
            </VField>
          </div>
        </div>
