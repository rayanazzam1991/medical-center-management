...
export default defineComponent({
    ...
    setup(props, context) {
        const clientReceipt = props.receipt
        const { t } = useI18n()
        ...
        const IQDAccountCashName = clientReceipt.entries.find((entry) => (entry.account.chart_account?.code
            == AccountConsts.CASH_CODE && entry.account.currency?.code ==
            CurrencyConsts.IQD_CODE))?.account.name ?? '-'
        const IQDAccountCurrencyRate = clientReceipt.entries.find((entry) => (entry.account.chart_account?.code
            == AccountConsts.CASH_CODE && entry.account.currency?.code ==
            CurrencyConsts.IQD_CODE))?.account.currency_rate ?? '-'
        const IQDAmount = clientReceipt.entries.find((entry) => (entry.account.chart_account?.code
            == AccountConsts.CASH_CODE && entry.account.currency?.code ==
            CurrencyConsts.IQD_CODE))?.amount ?? '-'
        const USDAccountCashName = clientReceipt.entries.find((entry) => (entry.account.chart_account?.code
            == AccountConsts.CASH_CODE && entry.account.currency?.code ==
            CurrencyConsts.USD_CODE))?.account.name ?? '-'
        const USDAccountCurrencyRate = USDAccountCashName != '-' ? clientReceipt.currency_rate : '-'
        const USDAmountInIQD = clientReceipt?.entries?.find(
            (entry) =>
                entry.account?.chart_account?.code === AccountConsts.CASH_CODE &&
                entry.account?.currency?.code === CurrencyConsts.USD_CODE
        )?.amount
        const USDAmount = USDAmountInIQD ? (Number(USDAmountInIQD.replace(/,/g, '')) / clientReceipt.currency_rate).toLocaleString('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        }) : '-'
        const currencyDifferenceAmount = clientReceipt.entries.find((entry) => (entry.account.chart_account?.code
            == ChartOfAccountConsts.CURRENCY_DIFFERENCES_CODE))?.amount ?? '-'

        return {
            t, clientReceipt, openModal, AccountConsts, CurrencyConsts,
        ...
        <template #content>
            <div class="columns is-multiline p-2">
                <div class="column is-12">
                    <p> {{ t('customer_cash_receipt.table.columns.client_name') }} : <span class="has-text-primary">{{
                        clientReceipt.entries.find((entry) =>
                            entry.account.chart_account?.code ==
                            AccountConsts.CLIENTS_CODE)?.account.name }}</span></p>
                </div>
                <div class="column is-12 columns p-0 m-0 py-2 px-5">
                    <div class="column is-4 columns is-multiline m-0 p-0 my-2 split-border">
                        ...
                        <div class="column is-12 split-border-bottom">
                            <p> {{ t('customer_cash_receipt.table.columns.account') }}</p>
                        </div>
                        <div class="column is-12 split-border-bottom">
                            <p> {{ t('customer_cash_receipt.table.columns.currency_rate') }}</p>
                        </div>
                        <div class="column is-12">
                            <p> {{ t('customer_cash_receipt.table.columns.cash_amount') }}</p>
                        </div>
                    </div>

                    <div class="column is-4 columns is-multiline m-0 p-0 my-2 split-border">
                        <div class="column is-12 split-border-bottom">
                            <p> {{ t('customer_cash_receipt.table.columns.iqd') }} </p>
                        </div>
                    ...
                    <div class="column is-4 columns is-multiline m-0 p-0 my-2">
                        <div class="column is-12 split-border-bottom">
                            <p> {{ t('customer_cash_receipt.table.columns.usd') }}</p>
                        </div>
                ...
                <div class="column is-12">
                    <p> {{ t('customer_cash_receipt.table.columns.currency_difference') }} : <span class="has-text-primary">
                            {{ addParenthesisToString(currencyDifferenceAmount) }}</span>
                    </p>
                </div>
                <div class="column is-12">
                    <p> {{ t('customer_cash_receipt.table.columns.action_date') }} : <span class="has-text-primary">
                            {{ clientReceipt.date }}</span>
                    </p>
                </div>
                <div class="column is-12">
                    <p> {{ t('customer_cash_receipt.table.columns.note') }} : <span class="has-text-primary">
                            {{ clientReceipt.note ?? '-' }}</span>
                    </p>
                </div>
            </div>
        </template>
