...
export interface ReceiptsDropDownProps {
    printPermission: string,
    detailsPermission: string,
}
const props = withDefaults(defineProps<ReceiptsDropDownProps>(), {
    printPermission: undefined,
    detailsPermission: undefined,
})
const printPermission = props.printPermission
const detailsPermission = props.detailsPermission
...
<template>
    <VDropdown icon="feather:more-vertical" class="is-pushed-mobile" spaced right>
        <template #content="{ close }">
            <a v-permission="detailsPermission" role="menuitem" href="#" class="dropdown-item is-media" @click.prevent="() => {
                emits('show')
                close()
            }
                ">
            ...
            <a v-if="!checkPermission(printPermission) && !checkPermission(detailsPermission)" role="menuitem"
