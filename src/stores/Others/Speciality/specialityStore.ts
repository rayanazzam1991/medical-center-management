...
import { activateSpecialityApi, deactivateSpecialityApi, getSpecialityApi, addSpecialityApi, editSpecialityApi, getSpecialitiesApi } from "/@src/utils/api/Others/Speciality"
...
export const useSpeciality = defineStore('speciality', () => {
    ...
    async function deactivateSpecialityStore(specialityId: number) {
        if (loading.value) return

        loading.value = true

        try {
            const response = await deactivateSpecialityApi(api, specialityId)
            specialities.value.splice(
                specialities.value.findIndex((speciality: Speciality) => speciality.id === specialityId),
                1
            )
    ...
    async function activateSpecialityStore(specialityId: number) {
        if (loading.value) return

        loading.value = true

        try {
            const response = await activateSpecialityApi(api, specialityId)
            specialities.value.splice(
                specialities.value.findIndex((speciality: Speciality) => speciality.id === specialityId),
                1
            )
    ...
    return {
        ...
        pagination,
        loading,
        deactivateSpecialityStore,
        activateSpecialityStore,
        addSpecialityStore,
        editSpecialityStore,
        getSpecialityStore,
