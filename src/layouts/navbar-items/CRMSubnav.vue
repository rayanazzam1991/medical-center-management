  ...
  <div class="navbar-subnavbar-inner tabs-wrapper" @mouseleave="() => emits('close')">
    ...
    t('crm_subnav.waiting_list.waiting_lists') }}</h4>
              ...
              <div class="column is-4" v-if="checkPermission(Permissions.REMINDER_CREATE) ||
                  ...
                  checkPermission(Permissions.RESERVATION_LIST) ||
                  checkPermission(Permissions.RESERVATION_CREATE)">
                ...
                </ul><br v-if="checkPermission(Permissions.REMINDER_CREATE) ||
                  checkPermission(Permissions.REMINDER_LIST)" />
                <h4 v-if="checkPermission(Permissions.RESERVATION_LIST) ||
                  checkPermission(Permissions.RESERVATION_CREATE)" class="column-heading">
                  {{ t('crm_subnav.reservation.reservations') }}</h4>
                <ul>
                  <li v-permission="Permissions.RESERVATION_CREATE">
                    <RouterLink to="/reservation-calendar">
                      <i class="fas fa-calendar-alt ml-2" aria-hidden="true"></i>
                      <span>{{ t('crm_subnav.reservation.reservation_calendar')
                      }}</span>
                      <i aria-hidden="true" class="iconify" data-icon="feather:circle"></i>
                    </RouterLink>
                  </li>
                  <li v-permission="Permissions.RESERVATION_LIST">
                    <RouterLink to="/reservation">
                      ...
                      <span>{{ t('crm_subnav.reservation.reservatons_list')
                      }}</span>
                      <i aria-hidden="true" class="iconify" data-icon="feather:circle"></i>
                    </RouterLink>
                  </li>
