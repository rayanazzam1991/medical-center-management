...
<template>
  ...
  <VModal :key="keyIncement" :title="t('employee_attendance.table.attendance_details_modal.title')" :open="tableCellPopup"
    actions="right" @close="tableCellPopup = false">
    <template #content>
  ...
  selectedCell.date
          ...
          <h2 class="is-size-7 mb-3">{{ t('employee_attendance.table.attendance_details_modal.status') }} <span
              class="has-text-primary is-size-6">{{
                t(`attendance_status.${AttendanceConsts.getAttendanceStatusName(selectedCell.status)
                  .replaceAll(' ', '_').toLowerCase()
                  }`)
              }}</span></h2>
        </div>
        <div>
          <VButton color="primary" v-if="canMarkAttendanceSelectedCell" raised @click="markAttendancePopup = true">{{
    ...
    <template #action="{ close }">
      <VButton :disabled="loading.update" v-if="(selectedCell.status == AttendanceConsts.PENDING_ABSENCE ||
        selectedCell.status == AttendanceConsts.PENDING_PARTIAL_ABSENCE) && canMarkAttendanceSelectedCell" class="mr-2"
        color="danger" outlined @click="() => {
          unjustifyAttendancePopup = true
      ...
      ) && canMarkAttendanceSelectedCell" class="mr-2" color="primary" outlined @click="() => {
  justifyAttendancePopup = true
  tableCellPopup = false
}">
  ...
  selectedCell.date
          ...
          <h2 class="is-size-7 mb-3">{{ t('employee_attendance.table.mark_attendance_modal.status') }} <span
              class="has-text-primary is-size-6">{{
                t(`attendance_status.${AttendanceConsts.getAttendanceStatusName(selectedCell.status).replaceAll(" ",
                  "_").toLowerCase()
                  }`)
              }}</span></h2>
        </div>
