...
const selectedStartTime = ref({ hours: '00', minutes: '00' })
const selectedEndTime = ref({ hours: '00', minutes: '00' })
...
const formatTime = () => {
  if (selectedCell.value.start_time) {
    ...
    selectedStartTime.value = { hours: Number(hour) < 10 && Number(hour) > 0 ? hour.substring(1) : hour, minutes: Number(minute) < 10 && Number(minute) > 0 ? minute.substring(1) : minute }

  }
  if (selectedCell.value.end_time) {
    const [hour, minute, second] = selectedCell.value.end_time.split(':')
    selectedEndTime.value = { hours: Number(hour) < 10 && Number(hour) > 0 ? hour.substring(1) : hour, minutes: Number(minute) < 10 && Number(minute) > 0 ? minute.substring(1) : minute }
  }
}
...
const updateSchedule = async () => {
  ...
  if (Number(selectedStartTime.value.minutes) < 10 && Number(selectedStartTime.value.minutes) > 0)
    formatedStartTimeMinute = '0' + selectedStartTime.value.minutes
  else
    formatedStartTimeMinute = selectedStartTime.value.minutes

  if (Number(selectedStartTime.value.hours) < 10 && Number(selectedStartTime.value.hours) > 0)
    formatedStartTimeHour = '0' + selectedStartTime.value.hours
  else
    formatedStartTimeHour = selectedStartTime.value.hours
  if (Number(selectedEndTime.value.minutes) < 10 && Number(selectedEndTime.value.minutes) > 0)
    formatedEndTimeMinute = '0' + selectedEndTime.value.minutes
  else
    formatedEndTimeMinute = selectedEndTime.value.minutes

  if (Number(selectedEndTime.value.hours) < 10 && Number(selectedEndTime.value.hours) > 0)
    formatedEndTimeHour = '0' + selectedEndTime.value.hours
  else
    formatedEndTimeHour = selectedEndTime.value.hours
          ...
          <div class="columns is-multiline">
            <div class="column is-12">
              ...
              <div class="columns">
                <VField class="column is-12 ">
                  <VControl>
                    <Datepicker v-model="selectedStartTime" :locale="locale" time-picker
            ...
            <div class="column is-12">
              ...
              <div class="columns ">
                <VField class="column is-12">
                  <VControl>
                    <Datepicker v-model="selectedEndTime" :locale="locale" time-picker
                      :cancel-text="t('date_picker.cancel')" :select-text="t('date_picker.select')" :dark="dark.isDark"
                      class="date-picker-dircetion" />
                  </VControl>
                </VField>
