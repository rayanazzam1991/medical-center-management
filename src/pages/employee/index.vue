...
import { dismissEmployeeHistory } from '/@src/services/Employee/employeeService'
...
<template>
  ...
  <VFlexTableWrapper :columns="columns" :data="employeesList" :limit="searchFilter.per_page" @update:sort="employeeSort">
    ...
    <VFlexPagination v-if="(employeesList.length != 0 && paginationVar.max_page != 1)" :current-page="paginationVar.page"
      class="mt-6" :item-per-page="paginationVar.per_page" :total-items="paginationVar.total" :max-links-displayed="3"
      no-router @update:current-page="getEmployeesPerPage" />
  ...
  <VModal :title="t('employee.table.modal_title.dismiss_employee')" :open="dismissEmployeePopup" actions="center"
    @close="dismissEmployeePopup = false">
    <template #content>
      <form class="form-layout" @submit.prevent="">
        <!--Fieldset-->
        <div class="form-fieldset">
    ...
    <template #action="{ close }">
      <VButton color="primary" raised @click="dismissEmployee()">{{ t('modal.buttons.confirm') }}</VButton>
    </template>
  </VModal>
</template>
